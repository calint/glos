...
inline static void metrics_print_headers(FILE *f) {
  fprintf(f,
          ...
          "%8s  %5s  %8s  %8s\n",
          "ms", "dt", "fps", "netlag", "nobj", "colp", "colc", "colf", "olchk",
          "upd", "rend", "pupd", "prend", "gtri", "nglo", "arrbufs", "texbufs");
}
...
inline static void metrics_print(FILE *f) {
  fprintf(f,
          ...
          " %06u  %06u  %08u"
          "  %05u  %08lu  %08u\n",
          metrics.tick, metrics.fps.dt, metrics.average_fps,
          metrics.net_lag_prv_frame, metrics.objects_allocated,
          ...
          metrics.objects_updated_prv_frame, metrics.objects_rendered_prv_frame,
          metrics.parts_updated_prv_frame, metrics.parts_rendered_prv_frame,
          metrics.triangles_rendered_prv_frame, metrics.glos_allocated,
          metrics.buffered_vertex_data, metrics.buffered_texture_data);
}
