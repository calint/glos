...
// engine interface
static inline auto application_print_hello() -> void { puts("\nglos\n"); }
...
static inline auto application_init() -> void {
    ...
    if (!is_performance_test) {
        ...
        float const skydome_scale =
            length(glm::vec2{grid_size / 2, grid_size / 2});
        ...
        case 2:
            ...
            break;
        default:
            throw glos::exception{std::format(
                "unknown 'performance_test_type' '{}'", performance_test_type)};
        }
    }
...
static inline auto create_cubes(uint32_t const num) -> void {
    ...
    for (uint32_t i = 0; i < num; ++i) {
        ...
        o->position = {rnd1(d), 0, rnd1(d)};
        o->linear_velocity = {rnd1(v), 0, rnd1(v)};
        o->angular_velocity.y = glm::radians(20.0f);
    }
}
